...
export default class AppStatusBar extends Component {
	...
	render() {
		...
		if(this.state.centerImage) {
			centerContent = (<Image source={this.props.centerIcon} style={styles.centerIcon} />)
		}
		else {
			centerContent = (<Text style={[styles.centerText, {color: this.props.textColor}]}>{this.props.text}</Text>)
		}
		return (
			<View style={[styles.statusBar, {backgroundColor: this.props.bColor}]} elevation={elevation}>
				<TouchableOpacity style={styles.iconContainer} onPress={this.props.leftFunction}>
					{ this.props.left ? <Image style={styles.iconImage} source={this.state.left} /> : null}
				</TouchableOpacity>
				<View style={styles.centerTextContainer}>
					{ this.props.center ? centerContent : null}
				</View>
				<TouchableOpacity style={styles.iconContainer} onPress={this.props.rightFunction}>
					{ this.props.right ? <Image style={styles.iconImage} source={this.state.right} /> : null}
				</TouchableOpacity>
			</View>
		);
	}
